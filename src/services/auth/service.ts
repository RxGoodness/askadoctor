...
export default {
  ...
  async createUser(params: IUser) {
    ...
    if (duplicate) {
      throw new APIError({
        status: 403,
        message:
          "A user with this email already exists. You can resend email verification if you are yet to verify your account",
  ...
  async sendVerification(params: { email: string }) {
    ...
    if (!user) {
      throw new APIError({
        status: 404,
        message:
          "user does not exist. Verification mail can only be sent to a user that has signed up. Proceed to sign up",
        path: "email",
      });
    }
  ...
  async verifyUser(params: { email: string; otp: string }) {
    ...
    if (!checkOtp) {
      throw new APIError({
        status: 403,
        message:
          "Invalid OTP supplied. Please request for a new OTP and use only the latest OTP to verify your account",
        path: "otp",
      });
    }
  ...
  async verifyLogin(params: { email: string; otp: string }) {
    ...
    if (!otpData) {
      throw new APIError({
        status: 404,
        message:
          "Invalid OTP or another OTP has been generated. Please only use the latest OTP to login",
        path: "otp",
      });
    }
    const isValid = await VendorLoginOtp.findOne({
      email: LCEmail,
      otp: otp,
      used: false,
    });
  ...
  async resetPassword(params: {
    ...
    const resetOtp = await VendorLoginOtp.findOne({
      email: LCEmail,
      otp: otp,
      used: false,
    });
    if (!resetOtp) {
      throw new APIError({
        status: 404,
        message:
          "Invalid OTP or OTP has been used or a new OTP has been generated. Please login again to generate a new OTP. Make sure you are using only the latest OTP sent to your email",
        path: "otp",
      });
    }
